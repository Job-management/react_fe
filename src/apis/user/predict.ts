...
const API_BASE_URL = process.env.REACT_APP_URL_PREDICT;
const apiPredict = axios.create({
  baseURL: API_BASE_URL,
});

apiPredict.interceptors.request.use(
  (config) => {
...
apiPredict.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error?.response?.status === 401) {
      ...
      try {
        ...
        return apiPredict(originalRequest);
      } catch (error) {
